...
8. 「src/gasToSlack.gs」を「実行」して、チャンネルに実際に使用されるフォームが送信されるかどうか確認する (認証が必要な場合は「許可」する)。成功した場合は、指定したチャンネルにフォームが送信される。送信に失敗した場合は、「チャンネルのID」「スプレッドシートのスクリプトプロパティ」「Google Apps Scriptのデプロイで「アクセスできるユーザー」を「全員」にしているかどうか」「ワークスペースおよびチャンネルに作成した「App」が「追加」されているかどうか」などの確認
